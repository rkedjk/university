...
bool SetClass_test::assertValue(int actual, int expected)
{
...
{
    if (passed)
        ++(this->testPassed), cout << "Успех!" << endl
                                   << endl;
    else
        cout << "Неудача!" << endl
             << endl;
...
bool SetClass_test::addOperationCreate()
{
    cout
        << "Тестируем метод AddOp:" << endl
        << "Производим создание множества" << endl;
    // Временная переменная для создания матрицы
    int Arr[50] = {1, 2, 3, 4, 5};
    Set Temp(5, Arr);
    return assertValue(strcmp(Temp.operations[0], "Created"), 0);
}

bool SetClass_test::addOperationAdd()
{   
     cout
        << "Тестируем метод AddOp:" << endl
        << "Добавляем элемент ко множеству" << endl;
    // arrange
    int Arr[50] = {1, 2, 3, 4, 5};
    Set Temp(5,Arr);
    ...
    return assertValue(strcmp(Temp.operations[1], "Added item"), 0);
}

bool SetClass_test::addOperationMul()
{
    cout
        << "Тестируем метод AddOp:" << endl
        << "Производим создание множества" << endl;
    ...
    return assertValue(strcmp(Temp.operations[1], "Created Set By Substraction"), 0);
}

// Проверка счётчика созданных объектов
bool SetClass_test::createdCount()
{
   cout
        << "Тестируем cчетчик созданных объектов:" << endl
        << "В процессе юнит тестирования создано н объектов" << endl;
    return assertValue(Set::getOverallCount(), 27);
}

// Проверка счётчика активных объектов
bool SetClass_test::activeCount()
{
    cout
        << "Тестируем cчетчик активных объектов:" << endl
        << "В процессе юнит тестирования все объекты удалены" << endl;
    return assertValue(Set::getActiveCount(), 0);
}
...
void SetClass_test::run()
{
    ...
    cout << "Программа выводит результаты автоматического Unit-тестирования" << endl
         << endl;
    startTest();
    ...
    updateTestResult(addOperationAdd());
	updateTestResult(addOperationCreate());
	updateTestResult(addOperationMul());
	updateTestResult(createdCount());
	updateTestResult(activeCount());
